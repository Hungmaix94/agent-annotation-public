...
## ⚡️ Tích Hợp Cho Project (Next.js & Vite)

Để Extension bắt được chính xác file path của thẻ HTML, bạn cần tích hợp Plugin.
Bây giờ việc cài đặt cực kỳ đơn giản với package `agent-annotator` có sẵn trên NPM!

**Cài đặt tự động (Khuyên dùng):**
Mở terminal ở thư mục gốc của project (chỗ chứa `package.json`) và chạy lệnh sau:

```bash
npx agent-annotator init
# (Hoặc nếu chưa publish NPM, bạn có thể chạy: npx github:Hungmaix94/agent-annotation-public agent-annotator-init)
```

Lệnh trên sẽ tự động dò tìm cấu hình project (Next.js hay Vite), cài đặt thư viện lõi, và in ra hướng dẫn ngắn gọn cho bạn gắn Plugin.

---

**Cài đặt thủ công:**
Nếu bạn muốn tự làm:
1. Cài package: `yarn add -D agent-annotator @babel/core @babel/preset-typescript`
2. Gắn plugin vào config:

*👉 Đối với Next.js (`next.config.mjs` hoặc `next.config.js`)*:
```javascript
const { withAgentAnnotator } = require('agent-annotator');
// hoặc: import { withAgentAnnotator } from 'agent-annotator';
...
Xong! Khởi động lại Server và Extension sẽ ngay lập tức bắt được chính xác đường dẫn của mọi Component (kể cả Next.js Server Components).
